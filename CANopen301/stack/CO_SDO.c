...
//#include 		"motor_control_protocol.h"		/*	opration motor interface */
...
/***************************************************************************
*		Why only exception_1003 is able write???
*
*/
uint16_t CO_OD_getAttribute(CO_SDO_t *SDO, uint16_t entryNo, uint8_t subIndex){
    ...
    if(object->maxSubIndex == 0U){   /* Object type is Var */
        ...
        /* Special exception: Object 1003,00 should be writable,author by ljb repair writeable */
    ...
    switch(state){
        ...
        case CO_SDO_ST_DOWNLOAD_INITIATE:{
            ...
            if((SDO->CANrxData[0] & 0x02U) != 0U){
                ...
                SDO->ODF_arg.data[3] = SDO->CANrxData[7];
								...
								/* motor control	by Object dictionary */
								MI_SetReg(pHandle, SDO);
								
                /* write data to the Object dictionary */
#if		( Enable_Write_Object	==	1	)							
        ...
        case CO_SDO_ST_UPLOAD_INITIATE:{
            ...
            SDO->CANtxBuff->data[3] = SDO->CANrxData[3];
						...
						/* get motor reg context	by Object dictionary */
						retVal	=	MI_GetReg(pHandle, SDO);					//rewrite SDO->ODF_arg.data[] data
						if( retVal != (uint32_t)GUI_ERROR_CODE ){
							SDO->ODF_arg.data[0]	=	retVal;
							SDO->ODF_arg.data[1]	=	retVal>>8;
							SDO->ODF_arg.data[2]	=	retVal>>16;
							SDO->ODF_arg.data[3]	=	retVal>>24;
						}
            /* Expedited transfer */
